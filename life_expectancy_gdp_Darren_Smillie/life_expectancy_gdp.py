...
# The 6 countries represented in the data are: Chile, China, Germany, Mexico, United States of America and Zimbabwe.
...
# The years 2000 to 2015, inclusive, are represented in the data.
...
f1, ax1 = plt.subplots(figsize=(10, 10)) # to try and better visualise the small numbers for Chile & Zimbabwe
sns.set_style("white")
sns.set_context("notebook")
sns.set_palette("Accent") # using a qualitative color pallette for these distinct but non-ordered categories
ax1 = sns.barplot(data=df,x="Country",y="GDP").set_title("Average GDP from 2000-2015 by country")
plt.show()
...
sns.set_context("notebook")
sns.set_palette("Accent") # using a qualitative color pallette for these distinct but non-ordered categories
ax1 = sns.barplot(data=df,x="Country",y="LEABY").set_title("Average life expectancy from 2000-2015 by country")
plt.show()
...
# The two charts look very different. At first glance, GDP doesn't appear correlated to life expectancy.
# A better measure may be GDP per person because of course GDP can be large based on a lot of people but they could still
# have a low quality of life which itself *could* be linked to life expectancy.
# Almost all of the countries have a very similar life expectancy on average, except Zimbabwe which is a real outlier,
# whereas, average GDP over the period varies dramatically between countries.
# Of course, we don't know from these charts how GDP and LE have varied over time so there could still be a relationship.
# Although we can see from the error bar, based on the distribution of LE over time, that it has changed most in Zimbabwe.
...
sns.set_palette("Accent") # using a qualitative color pallette for these distinct but non-ordered categories
sns.violinplot(data=df,x="Country",y="LEABY").set_title("Life expectancy distribution from 2000-2015 by country")
plt.ylabel("Life expectancy at birth (years)")
plt.savefig('LE distribution.png')
plt.show()
...
ax=sns.barplot(data=df,x="Country",y="GDP",hue="Year").set_title("GDP by country for the years 2000-2015")
plt.xticks(rotation=90)
plt.ylabel("GDP in Trillions of U.S. Dollars")
plt.show()

# Now zoom in on y-axis to see Zimbabwe's figures
f3, ax3 = plt.subplots(figsize=(10, 15))
ax3.set(ylim=(0, 20000000000))
ax3 =sns.barplot(data=df,x="Country",y="GDP",hue="Year").set_title("GDP by country for the years 2000-2015")
plt.xticks(rotation=90)
plt.ylabel("GDP in Trillions of U.S. Dollars")
plt.show()
...
f, ax = plt.subplots(figsize=(10, 15))
ax=sns.barplot(data=df,x="Country",y="LEABY",hue="Year").set_title("Life expectancy at birth by country for the years 2000-2015")
plt.xticks(rotation=90)
...
# All countries' GDP increases over the period, significantly. However, China's shows the biggest dramatic increase.
# Zimbabwe's GDP actually declines for the first half of the period before also dramatically increasing.
# Mexico shows probably the least relative GDP growth over the period, while Germany's seems to have plateaued in recent years.
# LE also increases for all countries over the period but in a smoother, more linear way compared to the almost "exponential"
# type growth seen in many cases with GDP.
# Interestingly, life expectancy does dip a little in Zimbabwe over a similar period to the fall in GDP while picking up
# afterwards. Life expectancy does appear to grow (relatively) the most for China and Zimbabwe where the relative growth of
# GDP was also the highest. So there does appear a suggestion of correlation (nothing to determine causation yet) but the 
# periods don't exactly align so this needs further investigation (more data, more countries to compare the trends, etc.).
...
g.fig.suptitle('Life expectancy vs GDP by country for each year between 2000 and 2015')
g.set_xlabels("GDP ($)")
g.set_ylabels("Life expectancy (years)")
g.set(xticks=[0,5000000000000,10000000000000,15000000000000,20000000000000])
plt.savefig('LE v GDP by country for each year.png')
plt.show()
...
# China moves most along the x-axis (GDP) over the period
# Zimbabwe moves most along the y-axis (LE) over the period (initially declining, then growing dramatically)
# This reflects what we identified above. It also reflects, at least in the GDP case, what we have heard in the new
# about the staggering economic growth of China.
# It's not as easy to read the charts as I'd like - you have to look across multiple separate plots and try and cross read the
# axes in order to spot the trends and it makes it difficult to judge accurately
...
plt.subplots_adjust(top=0.9)
g3.fig.suptitle('Life expectancy at birth by country from 2000 to 2015')
g3.set_ylabels("Life expectancy at birth (years)")
g3.set(xticks=[2000,2002,2004,2006,2008,2010,2012,2014])
plt.savefig('LE by country 2000-2015.png')
plt.show()
...
sns.set_context("notebook") # notebook
g3 = sns.FacetGrid(df, col="Country", col_wrap=3, size=4)
g3 = (g3.map(plt.plot, "Year", "GDP",color="blue").add_legend())
plt.subplots_adjust(top=0.9)
g3.fig.suptitle('GDP by country from 2000 to 2015')
g3.set_ylabels("GDP ($)")
g3.set(xticks=[2000,2002,2004,2006,2008,2010,2012,2014])
plt.savefig('GDP by country 2000-2015.png')
plt.show()
...
# The USA started and ended with the highest GDP while Zimbabwe started and ended with the lowest
...
# Zimbabwe started and ended with the lowest LE (though showed the most growth)
# Germany started and ended with the highest (closely followed by Chile and then the USA)
...
# Research published in the International Business & Economics Research Journal by Genius Murwirapachena, Nelson Mandela 
# Metropolitan University, and Courage Mlambo, University of Fort Hare, finds that the trend observed in the life expectancy in 
# Zimbabwe is attributable to economic factors. (https://www.researchgate.net/publication/297750732_Life_Expectancy_In_Zimbabwe_An_Analysis_Of_Five_Decades)
# They attribute the decline in life expectancy during the 2000-2009 decade to a decline in living standards driven by economic
# factors such as hyperinflation and "poor economic management policies". This subsequently led to food shortages, a decline in
# education and in health services.
# Unusually, male life expectancy fell by less than female life expectancy leading to males having a longer life expectancy over
# the period although the gap appears to be closing again.
# Having said all this, there was a very significant decline in life expectancy between the 1990s and 2000s in some surrounding 
# countries such as South Africa and Botswana. (This may also be attributable to the aids epidemic in the region.)
# Their paper notes that economic growth, inflation and population growth all correlate positively with life expectation in 
# Zimbabwe but other factors are also involved, for example increases in both agricultural land and the dependency ratio have a
# negative correlation with life expectancy in the country.
...
# Many factors affect life expectancy - diet, lifestyle, etc. However, all countries can increase their life expectancy
# through things like improved health, education and economic growth.
# The data really should be GDP per person otherwise a large economy might relate to a large population rather than reflect
# the quality of life of the people. Also, more countries are needed and data over a longer period of time to really draw
# statistically significant conclusions and exclude the many other individual factors that might impact an individual countries'
# GDP and life expectancy.
# None of these charts are ideal. A better chart than the FacetGrid line chart would be a single line chart with all of the 
# countries lines on it for an easier direct comparison, with lines color coded for identification.
# To spot the trend between GDP and LEABY we need both on the same chart ideally!
...
sns.set_style("white")
sns.set_context("notebook") # notebook
g4 = sns.FacetGrid(df, col="Country", hue="Year", col_wrap=3, size=4)
g4 = (g4.map(plt.scatter, "GDP", "LEABY", edgecolor="w").add_legend())
plt.subplots_adjust(top=0.9)
g4.fig.suptitle('Life expectancy vs GDP by country (2000-2015)')
g4.set_xlabels("GDP ($)")
g4.set_ylabels("Life expectancy at birth (years)")
plt.savefig('LE vs GDP by country 2000-2015.png')
plt.show()
